...
app.post("/regAction", (req, res) => {
  ...
  con.connect((err) => {
    if (err) {
      ...
      con.query(query, [name, email], async (err, result) => {
        if (err) {
          console.log("err", err.message);
        } else {
          ...
          con.query(query1, [name, hashpass], (err, result2) => {
            if (err) {
              console.log(err);
              ...
              con.query(query, async (err, result3) => {
                if (err) {
                  console.log("err", err.message);
        ...
        if (err) {
          ...
          if (result.length == 0) {
            ...
            if (await argon.verify(result[0].password, pass)) {
              ...
              con.query(query, async (err, result2) => {
                if (err) {
                  console.log("err", err.message);
                  return res.end("email does not exsit");
                } else {
